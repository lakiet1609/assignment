...
import argparse
import numpy as np
from pygments.lexer import line_re
from torch.fx.experimental.unification.multipledispatch.dispatcher import source
from vtkmodules.util.colors import linen
...
class ShotIdentification:
    ...
    @torch.no_grad()
    def run(self, poseweights='yolov7-w6-pose.pt', source='pose.mp4', device='gpu', names = 'utils/coco.names', line_thickness = 2):
                    ...
                    if len(output_data):  #check if no pose
                        ...
                        for det_index, (*xyxy, conf, cls) in enumerate(reversed(pose[:,:6])): #loop over poses for drawing on frame
                            ...
                            if len(sequence) == 30:
                                ...
                                if pose_name not in self.object_counter:
                                  self.object_counter[pose_name] = 1
                                else:
                                  self.object_counter[pose_name] += 1
